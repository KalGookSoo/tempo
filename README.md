...
크로스핏·웨이트 트레이닝을 위한 올인원 운동 타이머입니다. 타이머, 스톱워치, 인터벌을 앱 하나로 오가며 쓸 수 있는 iOS 네이티브 앱입니다.
...
- **하나의 앱, 세 가지 타이밍** — 타이머(카운트다운/카운트업), 스톱워치, 인터벌 타이밍을 한 앱에서 오가며 씁니다.
...
- **바로 쓰는 기본 프리셋** — Tabata, FGB, EMOM처럼 자주 쓰는 인터벌 구성이 기본으로 준비돼 있고, 그대로 실행하거나 복제해서 내 프리셋으로 바꿀 수 있습니다.
- **내 프리셋 만들기** — 운동/휴식 시간, 라운드 수, 준비 카운트다운까지 자유롭게 구성해 이름 붙여 저장합니다.
...
- **내 목소리로 알림 큐 만들기** — 구간이 바뀔 때 들려줄 소리를 사운드/진동은 물론, 직접 녹음한 음성(최대 5초, 실시간 파형 표시)으로도 지정할 수 있습니다.

<!-- docs/screenshots/*.png 경로에 스크린샷 이미지를 넣으면 위 표에 그대로 표시된다. 파일명 목록은 docs/screenshots/README.md 참고. -->

### 화면 미러링 대응

<img src="docs/screenshots/screen-mirroring.png" alt="화면 미러링 대응" width="30%">

제어센터 화면 미러링으로 TV에 연결하면, 폰 화면을 그대로 복제하는 대신 멀리서도 읽히는 전용 큰 화면을 보여줍니다.

> iOS의 화면 미러링은 Apple의 AirPlay 프로토콜을 사용합니다. TV가 AirPlay(2)를 지원해야 하고, 폰과 TV가 같은 Wi-Fi 네트워크에 있어야 합니다. 크롬캐스트 전용 TV처럼 AirPlay를 지원하지 않는 기기에는 미러링할 수 없습니다. 이 앱은 iOS 전용이라 안드로이드 기기에서는 애초에 사용할 수 없습니다.

### 잠금화면·다이나믹 아일랜드 실시간 표시

<img src="docs/screenshots/live-activity.png" alt="잠금화면·다이나믹 아일랜드 실시간 표시" width="30%">

앱을 벗어나도 Live Activity로 진행 상황을 계속 확인할 수 있습니다.

### 한국어·영어 지원

<img src="docs/screenshots/localization.png" alt="한국어·영어 지원" width="30%">

기기 언어 설정에 따라 자동으로 전환됩니다.

## 시작하기

1. `tempo/tempo.xcodeproj`를 Xcode로 엽니다.
2. `tempo` 스킴을 선택하고 시뮬레이터 또는 실기기에서 실행합니다.

별도 패키지 설치나 빌드 도구 설정은 필요 없습니다 — SwiftUI + SwiftData 표준 스택만 사용합니다.
