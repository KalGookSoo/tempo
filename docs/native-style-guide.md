...
- **다크 모드는 시스템이 처리한다**: 라이트/다크 모드별 색상을 직접 정의하지 않는다. 시스템 컬러(semantic color)를 쓰면 자동으로 전환된다. (예외: 상태 색상 4종 — "상태 색상" 절 참고)
...
`prepare`/`work`/`rest`/`danger` 4종은 시스템 시맨틱 컬러(`.yellow`/`.green`/`.teal`/`.red`)를 그대로 쓰지 않고, 라이트/다크 모드별로 명도·채도를 다르게 지정한 동적 색상(`Color.stateColor(light:dark:)`, `Views/RunningDisplayView.swift`)을 쓴다 — 시스템 색상(특히 노랑)이 라이트 모드에서 흰 배경 위 텍스트로 쓰였을 때 대비가 부족했기 때문이다. 다크 모드에서는 기존 시스템 색상을 그대로 쓴다. 이 4종 외의 색상(강조색, 배경, 텍스트 등)에는 이 예외를 적용하지 않는다 — 계속 시스템 시맨틱 컬러를 그대로 쓴다.
