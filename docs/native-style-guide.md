...
- **다크 모드 전용**: 기기의 시스템 설정(라이트/다크/자동)과 무관하게 앱은 항상 다크 모드로 표시된다(`.preferredColorScheme(.dark)`, 이슈 #85). 라이트 모드는 지원하지 않는다 — iOS 기본 시계 앱의 "타이머" 화면과 같은 정책이다. 라이트/다크 분기 색상을 직접 정의하지 않는 원칙은 그대로 유지한다: 시스템 컬러(semantic color)를 쓰면 다크 모드 값으로 항상 일관되게 렌더링된다.

## 컬러

커스텀 팔레트를 정의하지 않는다. 배경, 표면, 텍스트, 보더는 모두 시스템 시맨틱 컬러를 그대로 사용한다 (`Color.primary`, `Color.secondary`, `Color(.systemBackground)`,
`Color(.secondarySystemBackground)`, `Color(.separator)` 등). 앱이 다크 모드로 고정돼 있어 이 색상들은 항상 다크 모드 값으로 렌더링되므로 별도 팔레트 테이블이 필요 없다.
...
이 색상들은 Xcode의 Accent Color / Asset Catalog에 등록해서 쓰고, 하드코딩된 hex 값을 쓰지 않는다. 시스템 색상이므로 항상 다크 모드 기준의 적절한 톤으로 보정된다.
...
- 라이트/다크 분기 코드를 작성하지 않는다. 앱 전체가 `.preferredColorScheme(.dark)`로 고정돼 있고, 시스템 시맨틱 컬러를 쓰면 다크 모드 값으로 처리된다.
