...
- 분, 초를 네이티브 `Picker`(wheel style)로 설정한다.
...
- 저장된 인터벌 프로그램의 이름, 준비/운동/휴식 시간, 라운드를 수정한다.
- 화면 구성은 `.new`와 동일한 입력 폼(`IntervalConfigFormFields`)을 재사용한다.
- 세트의 운동/휴식 시간은 타이머 탭과 같은 휠 피커를 팝업(시트)으로 띄워 선택한다.
