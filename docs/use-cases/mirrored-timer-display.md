...
- 아이폰은 실행 화면을 포함해 세로 화면으로 고정된다(이슈 #54) — 아이폰으로
  미러링할 땐 폰을 세로로 든 채로 봐야 한다. 아이패드는 이 제약 없이 모든
  화면에서 세로/가로를 다 지원하므로, 눕혀서 보고 싶다면 아이패드를 쓴다.
...
- 아이폰에서는 가로 회전 자체가 안 되므로 가로 레이아웃 검수 대상이 아니다.
  아이패드에서만 가로 화면에서도 레이아웃이 깨지지 않는지 확인한다.
