...
/// 실행 화면(타이머/스톱워치/인터벌)이 지금 보여주고 있는 내용을, AirPlay 등으로
/// 연결된 외부 디스플레이(TV)에도 별도 창으로 그린다. 이슈 #63 참고.
///
/// `AirPlayButton`으로 사용자가 AirPlay 비디오 라우트를 선택하면 iOS가 외부
/// `UIScreen`을 만들어주는데, 폰 화면을 그대로 복제(제어센터의 화면 미러링)하는
/// 대신 이 화면이 그 `UIScreen` 위에 별도 창을 올려서 컨트롤 버튼 없이 표시
/// 전용으로 다시 그린다.
///
